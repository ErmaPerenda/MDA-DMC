...
class Population:
    ...
    def __init__(self,lambdaa,max_complexity=100000,num_classes=11,N_samples=256,max_block_depth=3,max_block_width=16,max_nbr_blocks=6,max_nbr_fcl=4,prob_dense=0.2):
    ...
    def save_the_best(self):
        indi = Individual(self.max_block_depth,self.max_block_width,self.max_nbr_blocks,self.max_nbr_fcl,self.prob_dense)
        indi.create_best_individual()
        with open('best_ind.dictionary', 'wb') as best_ind_dictionary_file:
            pickle.dump(indi, best_ind_dictionary_file)
