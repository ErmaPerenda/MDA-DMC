...
dataset_names=['big_iqimbalance.mat','big_iqimbalance.mat']#,'../../../../baseline_dataset1024_iqenergy_hardware_sco.mat']
...
def gen_data():
    ...
    for dataset_name in dataset_names:
        ...
        for key in ds.keys():
            ...
            m=pom[4]
            nt=int(pom[5])
            a_imb=int(pom[7].replace('neg','-'))
            ph_imb=int(pom[8].replace('neg','-'))
            ...
            if a_imb not in a_imbs:
                a_imbs.append(a_imb)

            if ph_imb not in ph_imbs:
                ph_imbs.append(ph_imb)
            ...
            c=np.full((total_len_a,6),[mod,snr,sps,nt,a_imb,ph_imb])
    ...
    print("nsps are ",nsps)
    print("amp imbs are ",a_imbs)
    print("phase imbs are ",ph_imbs)
...
if parse:
    ...
    np.save("a_imbs.npy",a_imbs)
    np.save("ph_imbs.npy",ph_imbs)
