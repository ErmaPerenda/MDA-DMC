...
def saveLatentMap(encoder,x_test, y_test,snr_mod_pairs_high):
    global mods
    
    lat = encoder.predict(x_test)
    x_train_encoded = TSNE(n_components=2).fit_transform(lat)
    # extract x and y coordinates representing the positions of the images on T-SNE plot
    tx = x_train_encoded[:, 0]
    ty = x_train_encoded[:, 1]
    ...
    colors_per_class=plt.get_cmap('tab20')
    j=1
    ...
    for mod in mods:
        ...
        indices = []
        i=0
        for snr_mod in snr_mod_pairs_high:
            if str(snr_mod[0])==str(mod):
                indices.append(i)
            i=i+1
        ...
        ax.scatter(current_tx, current_ty, alpha=0.5, color=colors_per_class(j), label=mod)
        j=j+1

    # build a legend using the labels we set previously
    ax.legend(loc='best', markerscale=6,prop={'size':30,'weight':'bold'})

    ax.tick_params(axis='x', labelsize=24)
    ax.tick_params(axis='y', labelsize=24)
    fig.savefig("images/tsne_map.png")
