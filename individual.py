...
class Individual:
    def __init__(self, max_block_depth=5, max_block_width=4,max_nbr_blocks=10, max_nbr_fcl=4,prob_dense=0.2, max_length=20):
        self.indi = []
        ...
        #self.m_prob=1
        self.c_prob=1.0
        self.margin=np.min([0.6+np.random.rand(),1])
        self.alpha=np.random.randint(0,100)/100.0
        beta_max=((1.0-self.alpha)*100)-10
        self.beta=1.0-self.alpha
        self.gama=1.0-(self.alpha+self.beta)
        ...
        #####################
        self.filters_range=[4,7] #num of filters is 2^ this range ie [4,8,16,32,64,128] 
    ...
    def create_best_individual(self):
        init_num_block=4
        _list=[]
        ...
        _list.append(self.add_first_block(init_width,init_depth))
        _list.append(self.add_second_block(init_width,init_depth))
        _list.append(self.add_third_block(init_width,init_depth))
        _list.append(self.add_fourth_block(init_width,init_depth))
   
            

        _list.append(self.add_a_globalavg_layer())
        dense_layer = DenseLayer(166,0.6)
        
        
        _list.append(self.add_a_dummy_layer())

        _list.append(dense_layer)
    ...
    def init_one_individual(self):
        ...
        init_num_block=np.random.randint(4,self.max_nbr_blocks+1)
        #init_num_block=6
        init_num_fcl=0
        init_num_fcl_class=0

        if np.random.rand()<self.prob_dense: 
            init_num_fcl=np.random.randint(0,self.max_nbr_fcl)
            #init_num_fcl=0

        prob_dense=1

        if np.random.rand()<prob_dense:
            init_num_fcl_class=np.random.randint(1,self.max_nbr_fcl+1)
        ...
        for _ in range(init_num_fcl_class):
            _list.append(self.add_a_random_dense_layer())
    ...
    def add_a_random_conv_layer(self,filters=-1):
        ...
        if filters==-1:
            filter_ind=np.random.randint(self.filters_range[0],self.filters_range[1])
    ...
    def add_first_block(self, width, depth):
        ...
        cnn_layer=ConvLayer(5, 32, 0.3)
        _list.append(cnn_layer)
        cnn_layer=ConvLayer(3, 16, 0.3)
        _list.append(cnn_layer)
        ...
        identity_conv=ConvLayer(3, 32, 0.3)
    ...
    def add_second_block(self, width, depth):
        ...
        cnn_layer=ConvLayer(5, 64, 0.3)
        _list.append(cnn_layer)
        cnn_layer=ConvLayer(1, 16, 0.1)
        _list.append(cnn_layer)

        merge_layer=self.add_a_random_merge_layer()
        pooling_layer=PoolLayer(2, 0.7)
        

        identity_apply_dimension_reduction=np.random.rand()
        

        identity_conv=ConvLayer(1, 32, 0.3)
    ...
    def add_third_block(self, width, depth):
        ...
        cnn_layer=ConvLayer(3, 32, 0.1)
        _list.append(cnn_layer)
        cnn_layer=ConvLayer(5, 16, 0.3)
        _list.append(cnn_layer)
        ...
        identity_conv=ConvLayer(3, 32, 0.9)
    ...
    def add_fourth_block(self, width, depth):
        ...
        cnn_layer=ConvLayer(3, 16, 0.1)
        _list.append(cnn_layer)
        cnn_layer=ConvLayer(1, 64, 0.6)
        _list.append(cnn_layer)
        ...
        identity_conv=ConvLayer(1, 128, 0.3)

        before_merge=False # apply reduction before block 
    ...
    def __str__(self):
        ...
        str_.append('margin:{} '.format(self.margin))
        str_.append('alpha:{} '.format(self.alpha))
        str_.append('beta:{} '.format(self.beta))
        str_.append('gama:{} '.format(self.gama))

        for i in range(self.get_layer_size()):
            unit = self.get_layer_at(i)
            if unit.type == 0:
                str_.append("\nBlock [width={},depth={}, {},".format(unit.width,unit.depth,str(unit.merge_layer)))
                pooling_bl=unit.pooling_layer
                if pooling_bl!=None:
                    str_.append("\npool[{},{}]".format(pooling_bl.kernel_size, "max" if pooling_bl.kernel_type<0.5 else "average"))
