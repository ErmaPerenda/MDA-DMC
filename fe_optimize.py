...
from evaluate_pure_class import Evaluate
from dataPreparation import get_data,get_no_augment_data,get_all_snr_data,get_mix_data
...
class r0739975:
	...
	def __init__(self, X_train_all, Y_train_all, X_valid_all, Y_valid_all,X_test, Y_test,snr_mod_pairs_test,snrs,mods,num_classes,N_samples):
		...
		self.numIters = 6			# Maximum number of iterations
		...
		self.max_block_width = 4 	# Block width
		self.max_block_depth = 3 	# Block depth
		self.max_nbr_blocks	= 7 	# Max Number of blocks
		self.prob_dense=0.99			# Probability to add a dense layer
		self.max_nbr_fcl = 2		# Max Number of Dense Layers
		...
		self.maxAcc=0.96           #accuracy for stopping of EA
		self.max_complexity=200000 #max allowed complexity for NN
	...
	def evaluate_fitness(self, gen_no):
		...
		evaluate = Evaluate(self.population,self.X_train_all, self.Y_train_all, self.X_valid_all,  self.Y_valid_all, self.X_test, self.Y_test, self.snr_mod_pairs_test, self.num_classes,self.N_samples,self.epochs, self.batch_size,self.max_complexity)
	...
	def save_best(self):
		...
		if idx !=-1:
			...
			evaluate = Evaluate(best_pop,self.X_train_all, self.Y_train_all, self.X_valid_all,  self.Y_valid_all, self.X_test, self.Y_test, self.snr_mod_pairs_test, self.num_classes,self.N_samples,self.epochs, self.batch_size,self.max_complexity)
		
			evaluate.evaluate_best(best_ind,self.X_train_all, self.Y_train_all, self.X_test,self.Y_test,self.X_valid_all, self.Y_valid_all,self.snr_mod_pairs_test,self.snrs,self.mods)
	...
	def optimize( self, plotFun = lambda x : None, plotMutation = lambda x : None):
		...
		for i in range(self.numIters):
			...
			best_ind=self.population.get_individual_at(0)
			with open('best_ind.dictionary', 'wb') as best_ind_dictionary_file:
				pickle.dump(best_ind, best_ind_dictionary_file)
	...
	def recombinate(self,X,gen_no):
		...
		evaluate=Evaluate(offspring_pops,self.X_train_all, self.Y_train_all, self.X_valid_all,  self.Y_valid_all, self.X_test, self.Y_test, self.snr_mod_pairs_test, self.num_classes,self.N_samples,self.epochs, self.batch_size,self.max_complexity)
		...
		for i in range(self.population.get_pop_size()):
			if dominatedCounts[i] == max_d or dominatedCounts[i] == 0:
				...
				for j in range(k):
					...
					evaluate = Evaluate(lo_pop,self.X_train_all, self.Y_train_all, self.X_valid_all,  self.Y_valid_all, self.X_test, self.Y_test, self.snr_mod_pairs_test, self.num_classes,self.N_samples,self.epochs, self.batch_size,self.max_complexity)
...
def plotPopulation2D(input):
	...
	for i in range(pop_size):
		...
		plt.plot(ind.accuracy,ind.complexity/10000.0,'o',color='blue')
...
#X_train_all, Y_train_all, X_valid_all, Y_valid_all,X_test, Y_test, snrs, mods,snr_mod_pairs_test=get_data()
#X_train_all, Y_train_all, X_valid_all, Y_valid_all,X_test, Y_test, snrs, mods,snr_mod_pairs_test=get_no_augment_data()
X_train_all, Y_train_all, X_valid_all, Y_valid_all,X_test, Y_test, snrs, mods,snr_mod_pairs_test=get_all_snr_data()
#X_train_all, Y_train_all, X_valid_all, Y_valid_all,X_test, Y_test, snrs, mods,snr_mod_pairs_test=get_mix_data()


print("\n\nTrain datasets have shapes such:")
print("Train Input dataset: ",X_train_all.shape)
print("Train Output dataset: ", Y_train_all.shape)
print("Valid Input dataset: ", X_valid_all.shape)
print("Valid Output dataset: ", Y_valid_all.shape)
print("snr mod paris ",snr_mod_pairs_test.shape)

print("\n\n")

start = time.time()
dnn_opt = r0739975(X_train_all, Y_train_all, X_valid_all, Y_valid_all,X_test, Y_test,snr_mod_pairs_test,snrs,mods,len(mods),128)
		
evaluate_only_best=True
if evaluate_only_best==False:
	dnn_opt.optimize(plotPopulation2D,plotMutationRate)
else:
	best_pop=Population(0)
	best_pop.save_the_best()

	with open('best_ind.dictionary', 'rb') as best_ind_dictionary_file:
		best_ind= pickle.load(best_ind_dictionary_file)
		best_pop=Population(0)
		best_pop.append_individual(best_ind)
		evaluate = Evaluate(best_pop,X_train_all, Y_train_all, X_valid_all, Y_valid_all, X_test, Y_test,snr_mod_pairs_test, len(mods),128,80, 256,1000000)
	
		evaluate.evaluate_best(best_ind,X_train_all, Y_train_all, X_test,Y_test,X_valid_all, Y_valid_all,snr_mod_pairs_test,snrs,mods)
